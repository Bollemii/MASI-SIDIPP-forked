...
class TestJoinCommunity:
    ...
    def test_send_public_key_to_response_to_send_invitation(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
    ...
    def test_received_auth_key_failed(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
        """Test if the auth key is not received"""
        member = tuple(["127.0.0.1", 1111])
        mock_client.receive_message.side_effect = [
            tuple([MessageDataclass(MessageHeader.DATA, "public_key"), member]),
            (None, None),
    ...
    def test_received_symetric_key_failed(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
        """Test if the symetric key is not received"""
        member = tuple(["127.0.0.1", 1111])
        mock_client.receive_message.side_effect = [
            tuple([MessageDataclass(MessageHeader.DATA, "public_key"), member]),
            tuple([MessageDataclass(MessageHeader.DATA, "encr_auth_key"), member]),
        ...
        assert result != "Success!"
        assert "No symetric key received" in result
        join_community_use_case.community_repository.add_community.assert_not_called()

    def test_received_symetric_key_reject_message(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
        """Test if we receive reject message"""
        member = tuple(["127.0.0.1", 1111])
        mock_client.receive_message.side_effect = [
            tuple([MessageDataclass(MessageHeader.DATA, "public_key"), member]),
            tuple([MessageDataclass(MessageHeader.DATA, "encr_auth_key"), member]),
            tuple([MessageDataclass(MessageHeader.REJECT, "Error message"), member]),
        ]
    ...
    def test_received_community_informations_failed(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
        """Test if the community informations are not received"""
        member = tuple(["127.0.0.1", 1111])
        mock_client.receive_message.side_effect = [
            tuple([MessageDataclass(MessageHeader.DATA, "public_key"), member]),
            tuple([MessageDataclass(MessageHeader.DATA, "encr_auth_key"), member]),
        ...
        assert result != "Success!"
        assert "No community informations received" in result
        join_community_use_case.community_repository.add_community.assert_not_called()
    ...
    def test_received_community_database_failed(
        self, mock_client: MagicMock, join_community_use_case: JoinCommunity
    ):
        """Test if the community database is not received"""
        member = tuple(["127.0.0.1", 1111])
        mock_client.receive_message.side_effect = [
            tuple([MessageDataclass(MessageHeader.DATA, "public_key"), member]),
            tuple([MessageDataclass(MessageHeader.DATA, "encr_auth_key"), member]),
        ...
        assert result != "Success!"
        assert "No community database received" in result
        join_community_use_case.member_repository.update_member_relationship.assert_not_called()
